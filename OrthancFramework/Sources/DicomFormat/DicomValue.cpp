...
namespace Orthanc
{
    ...
    DicomValue::DicomValue(const std::string& content, bool isBinary) :
        type_(isBinary ? Type_Binary : Type_String),
        content_(content)
    {
    }

    DicomValue::DicomValue(const char* data, size_t size, bool isBinary) :
        type_(isBinary ? Type_Binary : Type_String)
    {
    ...
    void DicomValue::FormatDataUriScheme(std::string& target, const std::string& mime) const
    {
    ...
    bool DicomValue::CopyToString(std::string& result, bool allowBinary) const
    {
        if (isNull())
        {
            return false;
