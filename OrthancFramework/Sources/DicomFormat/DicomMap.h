...
namespace Orthanc
{
    ...
    private:
        ...
        friend class FromDcmtkBrighe;
        friend class ParseDicomFiles;
    ...
    public:
        ...
        void SetNullValue(uint16_t group, uint16_t element);

        void SetNullValue(const DicomTag& tag);

        void SetValue(uint16_t group, uint16_t element, const DicomValue& value);

        void SetValue(const DicomTag& tag, const DicomValue& value);

        void SetValue(const DicomTag& tag, const std::string& str, bool isBinary);

        void SetValue(uint16_t group, uint16_t element, const std::string& str, bool isBinary);

        void SetValue(const DicomTag& tag, const Json::Value& value);
        ...
        void CopyTagIfExists(const DicomMap& source, const DicomTag& tag);
        ...
        // returns a string uniquely identifying the list of main dicom tags of a level
        ...
        static const std::set<DicomTag>& GetAllMainDicomTag();
        ...
        static bool IsDicomFile(const void* dicom， size_t size);

        static bool ParseDicomMetaInformation(DicomMap& result, const void* dicom, size_t size);
        ...
        bool LookupStringValue(std::string& result, const DicomTag& tag, bool allowBinary) const;

        bool ParseInteger32(int32_t& result, const DicomTag& tag) const;

        bool ParseInteger64(int64_t& result, const DicomTag& tag) const;

        bool ParseUnsignedInteger32(uint32_t& result, const DicomTag& tag) const;

        bool ParseUnsignedInteger64(uint64_t& result, const DicomTag& tag) const;

        bool ParseFloat(float& result, const DicomTag& tag) const;

        bool ParseFirstFloat(float& result, const DicomTag& tag) const;

        bool ParseDouble(double& result, const DicomTag& tag) const;
        ...
        void MergeMainDicomTags(const DicomMap& other, ResourceType level);
