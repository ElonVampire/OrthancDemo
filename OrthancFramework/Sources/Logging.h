...
namespace Orthanc
{
    namespace Logging
    {
        ...
        ORTHANC_PUBLIC void SetCategoryEnabled(LogLevel level, LogCategory category, bool enabled);

        ORTHANC_PUBLIC bool IsCategoryEnabled(LogLevel level, LogCategory category);

        ORTHANC_PUBLIC bool LookupCategory(LogCategory& target, const std::string& category);
...
namespace Orthanc
{
    namespace Logging
    {
        ...
        private:
            ...
            void Setup(LogCategory category, const char* file, int line);
        
        public:
            InternalLogger(LogLevel level, LogCategory category, const char* file, int line);

            // For backward binary compatibility with Orthanc Framework <= 1.8.0
            InternalLogger(LogLevel level, const char* file, int line);
